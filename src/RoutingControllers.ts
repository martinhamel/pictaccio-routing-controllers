...
export class RoutingControllers<T extends BaseDriver> {
  ...
  registerControllers(classes?: Function[]): this {
    ...
    controllers.forEach(controller => {
      controller.actions.forEach(actionMetadata => {
        ...
        const viewAction = getMetadataArgsStorage().responseHandlers
          .filter(handler => handler.type === 'view' && handler.method === actionMetadata.method)[0];
        const controllerInstance = controller.getInstance(actionMetadata as unknown as Action);

        if (this.options.viewClass) {
          actionMetadata.view = controllerInstance.view =
            // @ts-ignore
            controllerInstance.view || new this.options.viewClass(controllerInstance);
        }
        this.driver.registerAction(actionMetadata, (action: Action) => {
          return this.executeAction(actionMetadata, action, viewAction, interceptorFns);
        });
      });
    });
   ...
   */
  protected executeAction(actionMetadata: ActionMetadata, action: Action, viewAction: ResponseHandlerMetadataArgs, interceptorFns: Function[]) {
    ...
    return Promise.all(paramsPromises)
      .then(params => {
        ...
        const result = viewAction
          ? actionMetadata.view.executeAction(actionMetadata, action, allParams)
          : actionMetadata.methodOverride
